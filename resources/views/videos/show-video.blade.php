        ...
        <div class="mx-auto col-10">
            ...
            @endforeach
                ...
                <select id="qualityPick">
                    ...
                    <option value="240" {{$video->quality == 240 ? 'selected' : ''}} {{$video->quality < 240 ? 'hidden':''}}>240p</option>                    
                </select>
                ...
                <div class='interaction text-center mt-5'>
                    <a href="" class="like ml-3">
                        @if($userLike)
                            @if($userLike->like == 1)
                                <i class="far fa-thumbs-up fa-2x liked"></i> <span id='likedNumber'>{{$countLike}}</span>
                            @else 
                                <i class="far fa-thumbs-up fa-2x"></i> <span id="likedNumber">{{$countLike}}</span>
                            @endif
                        @else
                    ...
                    @endforeach
                    {{--  loginAlert  --}}
                    <div class="loignAlert"></div>
                </div>
                {{-- comments section started here  --}}
                <div class='mt-4 px-2'>
                    <div class="comments">
                        <div class="mb-3">
                            <span>التعليقات</span>
                        ...
                        <div>
                            <textarea class="form-control" id="comment" rows="4" placeholder="اضافة تعليق عام"></textarea>
                            <button type="submit" class="btn btn-outline-primary mt-3 saveComment">تعلق</button>
                            <div class="commentAlert mt-5">

                            </div>
                            {{-- get all video's comment --}}
    ...
    $('.like').on('click', function(event){
        ...
        if(AuthUser == '1'){
            ...
            var html = `<div class="alert">
                            <ul>
                                <li class="loginAlert">You must to logging in</li>
                            </ul>
                        </div>`;
...
{{-- end like script  --}}
{{-- video views script --}}
{{--  you can choose what ever way you want --}}
{{-- 1- when the user click play video --}}
{{-- 2- when the  user watch half the video  --}}
{{-- 3- when the user watch the whole video --}}
<script>
// ended when the video finshed
$('#videoPlayer').on('ended', function(e){
    ...
    $.ajax({
        ...
        success: function(data){
            $('.viewsNumber').html(data.viewsNumber);
        }
    ...
    $('.saveComment').on('click', function(e) {
        ...
        if(AuthUser == '1'){
            var html = `<div class="alert alert-danger">
                    <ul>
                        <li class="loginAlert">لا يجب تسجيل الدخول لكي تستطيع التعليق على الفيديو </li>
                    </ul>
                </div>`;
                ...
                var html = `<div class="alert alert-danger">
                    <ul>
                        <li class="loginAlert">انت ممنوع من الإعجاب  </li>
                    </ul>
                </div>`;
                $('.commentAlert').html(html);  
            ...
            var html = `<div class="alert alert-danger">
                    <ul>
                        <li class="loginAlert">يجب اضافة التعليق  لكي تتم العملية </li>
                    </ul>
                </div>`;
                console.log(html);
                $('.commentAlert').html(html);
            ...
            $("commentAlert").html('');
            videoId = $('#videoId').val(); // get the video id
                    ...
                    var html=`  <div class="card mt-5 mb-3">
                                    <div class="card-body">
                                        <div class="row">
                                            ...
                                            <div class="col-10">
                                                <form method="GET" action="${destroy}" onsubmit="return confirm('هل أنت متأكد أنك تريد حذف التعليق هذا؟')">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="float-left"><i class="far fa-trash-alt text-danger fa-lg"></i></button>
                                                </form>
